...
<table id="my" width="700" border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse" align="center">
    ...
    <form action="/web_war_exploded/modUser" method="post" enctype="multipart/form-data">
        ...
        <tr>
            <td>
                请输入你的爱好：
                <input type="checkbox" name="txtLOVE" value="阅读" <c:if test="${requestScope.userLikes.isContain('阅读')}">checked</c:if>>
                阅读&nbsp;
                <input type="checkbox" name="txtLOVE" value="音乐" <c:if test="${requestScope.userLikes.isContain('音乐')}">checked</c:if> >
                音乐&nbsp;
                <input type="checkbox" name="txtLOVE" value="运动" <c:if test="${requestScope.userLikes.isContain('运动')}">checked</c:if>>
                运动&nbsp;
                <input type="checkbox" name="txtLOVE" value="其他" <c:if test="${requestScope.userLikes.isContain('其他')}">checked</c:if> >
                其他&nbsp;
            </td>
        </tr>
        <tr>
